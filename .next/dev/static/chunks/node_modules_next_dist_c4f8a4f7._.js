...
function createServerSearchParamsForMetadata(underlyingSearchParams, workStore) {
...
function createServerSearchParamsForServerPage(underlyingSearchParams, workStore, varyParamsAccumulator = null) {
    const workUnitStore = _workunitasyncstorageexternal.workUnitAsyncStorage.getStore();
    if (workUnitStore) {
        switch(workUnitStore.type){
            case 'prerender':
            ...
            case 'prerender-runtime':
                return createRuntimePrerenderSearchParams(underlyingSearchParams, workUnitStore, varyParamsAccumulator);
            case 'request':
                return createRenderSearchParams(underlyingSearchParams, workStore, workUnitStore);
            default:
                workUnitStore;
...
function createRuntimePrerenderSearchParams(underlyingSearchParams, workUnitStore, varyParamsAccumulator) {
    ...
    return (0, _dynamicrendering.delayUntilRuntimeStage)(workUnitStore, makeUntrackedSearchParams(underlyingSearchParamsWithVarying));
}
...
function createServerParamsForMetadata(underlyingParams, workStore) {
    const metadataVaryParamsAccumulator = (0, _varyparams.getMetadataVaryParamsAccumulator)();
    return createServerParamsForServerSegment(underlyingParams, workStore, metadataVaryParamsAccumulator);
}
function createServerParamsForRoute(underlyingParams, workStore, varyParamsAccumulator = null) {
    const workUnitStore = _workunitasyncstorageexternal.workUnitAsyncStorage.getStore();
    if (workUnitStore) {
        switch(workUnitStore.type){
            case 'prerender':
            ...
            case 'prerender-runtime':
                return createRuntimePrerenderParams(underlyingParams, workUnitStore, varyParamsAccumulator);
            case 'request':
                if ("TURBOPACK compile-time truthy", 1) {
                    // Semantically we only need the dev tracking when running in `next dev`
                    // but since you would never use next dev with production NODE_ENV we use this
...
function createServerParamsForServerSegment(underlyingParams, workStore, varyParamsAccumulator = null) {
    const workUnitStore = _workunitasyncstorageexternal.workUnitAsyncStorage.getStore();
    if (workUnitStore) {
        switch(workUnitStore.type){
            case 'prerender':
...
function createRuntimePrerenderParams(underlyingParams, workUnitStore, varyParamsAccumulator) {
    const underlyingParamsWithVarying = varyParamsAccumulator !== null ? (0, _varyparams.createVaryingParams)(varyParamsAccumulator, underlyingParams) : underlyingParams;
    return (0, _dynamicrendering.delayUntilRuntimeStage)(workUnitStore, makeUntrackedParams(underlyingParamsWithVarying));
}
...
//# sourceMappingURL=node_modules_next_dist_c4f8a4f7._.js.map