...
const Archives: React.FC = () => {
  ...
  const loadItems = async () => {
    ...
    if (activeTab === ArchiveType.VISION_5Y || activeTab === ArchiveType.LIFE_TODO) {
      ...
      if (found) selectItem(found);
        ...
        await supabaseService.updateArchive(newLifeTodo); setItems((prev: ArchiveItem[]) => [...prev, newLifeTodo]); selectItem(newLifeTodo);
      }
    }
  };
  ...
  const handleSave = async () => {
    if (selectedItem) {
      ...
      await supabaseService.updateArchive(updatedItem);
      setItems((prev: ArchiveItem[]) => prev.map((i: ArchiveItem) => i.id === updatedItem.id ? updatedItem : i));
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden rounded-2xl border border-graphite-200 dark:border-graphite-800 bg-white dark:bg-graphite-900 shadow-sm">
        ...
        <div className="w-64 flex-shrink-0 border-r border-graphite-100 dark:border-graphite-800 bg-graphite-50 dark:bg-graphite-950 p-4 space-y-1">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSelectedItem(null); if (tab.id === ArchiveType.VISION_5Y || tab.id === ArchiveType.LIFE_TODO) { const found = items.find((i: ArchiveItem) => i.type === tab.id); if (found) selectItem(found); } }}
