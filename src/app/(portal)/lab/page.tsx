                            ...
                            {isExpanded && (
                                <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-top-4 duration-300">
                                    <div className="space-y-8">
                                        ...
                                        {cycle.mantras?.length > 0 && (
                                            <div>
                                                <h4 className={`text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2 ${isSystemActive ? 'text-graphite-500' : 'text-graphite-500 dark:text-graphite-400'}`}><Brain size={14} /> Current Mantras</h4>
                                                <div className="space-y-3">
                                                    {cycle.mantras.map((m: string, i: number) => (
                                                        <div key={i} className="flex items-start gap-4 p-3 rounded-xl border bg-pacific-50/50 dark:bg-pacific-900/10 border-pacific-200/50 dark:border-pacific-800/30 transition-all hover:border-pacific-400/50 dark:hover:border-pacific-600/40">
                                                            <span className="text-pacific-500 font-mono text-xs font-bold mt-0.5 shrink-0">{String(i + 1).padStart(2, '0')}.</span>
                                                            <span className="text-sm font-medium italic text-graphite-700 dark:text-graphite-300">"{m}"</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
