...
import { Slide, SlideCategory } from '@/types/types';
...
const Cinema: React.FC = () => {
  ...
  useEffect(() => {
    supabaseService.getSlides().then((fetchedSlides: Slide[]) => {
      const mergedSlides = categoryOrder.map((cat: SlideCategory) => {
        const existing = fetchedSlides.find((s: Slide) => s.category === cat);
        return existing || {
          id: `temp-${cat}`,
          category: cat,
  ...
  const handleSave = async () => {
    if (activeIndex !== null) {
      ...
      setSlides((prev: Slide[]) => prev.map((s: Slide, i: number) => i === activeIndex ? updatedSlide : s));
      setIsEditing(false);
    }
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onloadend = () => {
        setEditForm((prev: any) => ({ ...prev, image_url: reader.result as string }));
      };
            ...
            <div className="flex-1 relative flex flex-col items-center justify-center p-8 overflow-hidden bg-graphite-50 dark:bg-black/40">
              ...
              {(activeSlide.gallery_images && activeSlide.gallery_images.length > 0) && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 h-20 flex gap-2 overflow-x-auto p-2 bg-white/60 dark:bg-graphite-900/60 backdrop-blur-xl rounded-xl border border-graphite-200 dark:border-white/10 shadow-lg">
                  {activeSlide.gallery_images.map((url: string, i: number) => (
