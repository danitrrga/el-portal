...
import { Goal, GoalType, Version, Cycle, HabitWeight, Habit, EntityStatus } from '../types';
import { Target, Zap, Circle, CheckCircle2, Plus, ArrowRight, Brain, AlertTriangle, List, X, Layers, Activity, Fingerprint, ChevronRight, BarChart3, Calendar, ChevronDown, Edit3, Save, Trash2, Check, LayoutList, Link as LinkIcon, ChevronUp, MoreHorizontal, Copy, History, Link, RefreshCw, Search, CornerDownRight, Archive, Eye, EyeOff } from 'lucide-react';
            ...
            <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-graphite-900 shadow-2xl animate-in zoom-in-95 duration-200 border border-graphite-200 dark:border-graphite-800 flex flex-col max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
                <div className="p-6 border-b border-graphite-200 dark:border-graphite-800">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg font-bold text-graphite-900 dark:text-white flex items-center gap-2"><Link size={18} className="text-pacific-500"/> Link Habit</h2>
...
const Lab: React.FC = () => {
  ...
  const loadLabData = async () => {
      try {
          const labData = await db.getLabData();
          setData(labData);
          const history = await db.getAllHabits();
  ...
  const handleCreateVersion = async (vData: { title: string; description: string; number: number; startDate: string }) => { await db.createVersion(vData); setShowNewVersion(false); loadLabData(); };
  
  const handleCreateCycle = async (cData: any, gData: any[], hData: any[]) => { 
      await db.createCycle(cData, gData, hData); 
      // If cycle status is active, update DB
      if (cData.status === EntityStatus.ACTIVE) {
          // Note: MockDB creates ID internally, we rely on the backend setting it active if passing active status
          // The current mockDb.createCycle sets profile.current_cycle_id automatically
      }
      setShowNewCycle(false); 
      loadLabData(); 
  };

  const handleUpdateCycle = async (updatedCycle: Cycle) => { await db.updateCycle(updatedCycle); setEditingCycle(null); loadLabData(); };
  ...
  const handleArchiveCycle = async (e: React.MouseEvent, cycleId: string) => {
    ...
    try {
        ...
        const updatedCycles = data.cycles.map(c => c.id === cycleId ? { ...c, status: newStatus } : c);
        setData({ ...data, cycles: updatedCycles });
        
        await db.updateCycle({ ...cycle, status: newStatus });
        await loadLabData();
    } catch (e) {
  ...
  const handleUpdateCycleSettings = async (cycleId: string, startDate: string, endDate: string, setActive: boolean) => {
    ...
    if (cycleToUpdate) {
        ...
        await db.updateCycle(updatedCycle);
    }
    if (setActive && data.activeCycleId !== cycleId) { await db.setCycleActive(cycleId); }
      ...
      if (goalData.type === GoalType.TASK_PROJECT) {
          ...
          newGoal.subtasks = tasks;
           ...
           if (linkedHabit.cycle_id !== targetCycleIdForAddGoal) {
               ...
               await db.addHabit(newHabit); habitId = newHabitId;
           } else { const updatedHabit = { ...linkedHabit, linked_goal_id: newGoalId }; await db.updateHabit(updatedHabit); }
           newGoal.linked_habit_id = habitId; newGoal.current_streak = 0;
      }
      await db.addGoal(newGoal); setShowAddGoalModal(false); loadLabData();
  };

  const handleDeleteGoal = async (id: string) => { await db.deleteGoal(id); loadLabData(); };
  ...
  const handleLinkHabit = async (habit: Habit) => {
      ...
      if (!isCurrentCycle && data.activeCycleId) {
          ...
          await db.addHabit(newHabit); habitIdToLink = newHabit.id;
      } else { const updatedHabit = { ...habit, linked_goal_id: selectedGoalIdForLinking }; await db.updateHabit(updatedHabit); }
      const goalToUpdate = data.goals.find(g => g.id === selectedGoalIdForLinking);
      if (goalToUpdate) { const updatedGoal = { ...goalToUpdate, linked_habit_id: habitIdToLink }; await db.updateGoal(updatedGoal); }
      setShowHabitSelector(false); setSelectedGoalIdForLinking(null); loadLabData();
  };

  const handleToggleSubtask = async (goal: Goal, index: number) => { if (!goal.subtasks) return; const updatedSubtasks = [...goal.subtasks]; updatedSubtasks[index].done = !updatedSubtasks[index].done; const updatedGoal = { ...goal, subtasks: updatedSubtasks }; await db.updateGoal(updatedGoal); loadLabData(); };
  const handleDeleteSubtask = async (goal: Goal, index: number) => { if (!goal.subtasks) return; const updatedSubtasks = goal.subtasks.filter((_, i) => i !== index); const updatedGoal = { ...goal, subtasks: updatedSubtasks }; await db.updateGoal(updatedGoal); loadLabData(); };
  ...
  const handleSaveSubtaskEdit = async (goal: Goal, index: number) => { if (!goal.subtasks) return; const updatedSubtasks = [...goal.subtasks]; updatedSubtasks[index].name = editSubtaskText; const updatedGoal = { ...goal, subtasks: updatedSubtasks }; await db.updateGoal(updatedGoal); setEditingSubtask(null); setEditSubtaskText(''); loadLabData(); };
  const handleAddNewSubtask = async (goal: Goal) => { if (!newSubtaskText.trim()) return; const updatedSubtasks = goal.subtasks ? [...goal.subtasks] : []; updatedSubtasks.push({ name: newSubtaskText, done: false, level: 0 }); const updatedGoal = { ...goal, subtasks: updatedSubtasks }; await db.updateGoal(updatedGoal); setAddingSubtaskGoalId(null); setNewSubtaskText(''); loadLabData(); };
  ...
  const handleManageHabit = async (action: 'create' | 'update' | 'delete', payload: any) => {
      if (!editingCycle) return;
      if (action === 'delete') {
          await db.deleteHabit(payload.id);
  ...
  const handleRenameCategory = async (oldName: string, newName: string) => {
      ...
      for (const h of toUpdate) {
          await db.updateHabit({ ...h, category: newName });
      }
      await loadLabData();
  };
  ...
  const handleDeleteCategory = async (categoryName: string) => {
      ...
      for (const h of toUpdate) {
          await db.updateHabit({ ...h, category: 'Uncategorized' });
      }
      await loadLabData();
  };
                     ...
                     {isExpanded && (
                         <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-top-4 duration-300">
                             <div className="space-y-8">
                                 <div><h4 className={`text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2 ${isSystemActive ? 'text-graphite-500' : 'text-graphite-500 dark:text-graphite-400'}`}><Target size={14}/> Priorities</h4><div className="space-y-3">{cycle.focus_priorities.map((p, i) => (<div key={i} className={`flex items-center gap-4 p-3 rounded-xl border ${isSystemActive ? 'bg-white/5 border-white/10' : 'bg-graphite-50 dark:bg-graphite-800 border-graphite-200 dark:border-graphite-700'}`}><span className="text-pacific-500 font-mono text-xs font-bold">0{i+1}.</span><span className={`font-medium ${textBaseClass}`}>{p}</span></div>))}</div></div>
                                 <div><h4 className={`text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2 ${isSystemActive ? 'text-graphite-500' : 'text-graphite-500 dark:text-graphite-400'}`}><Activity size={14}/> Identity Kernel</h4><div className="grid grid-cols-2 gap-4">{cycle.cch_list.map((c, i) => (<div key={i}><div className={`text-[10px] font-bold mb-2 uppercase ${isSystemActive ? 'text-graphite-500' : 'text-graphite-600 dark:text-graphite-400'}`}>{c.category}</div><div className="flex flex-wrap gap-2">{c.items.map((item, j) => (<span key={j} className={`text-xs border px-2 py-1 rounded-md ${isSystemActive ? 'text-graphite-400 border-graphite-700' : 'text-graphite-400 dark:text-graphite-500 border-graphite-200 dark:border-graphite-700'}`}>{item}</span>))}</div></div>))}</div></div>
                                 ...
                                 <div className="pt-6 border-t border-graphite-200 dark:border-graphite-800 flex flex-wrap items-center gap-3 relative z-10" onClick={(e) => e.stopPropagation()}>
      ...
      {editingCycle && (
          <CycleEditorModal
              ...
              onRenameCategory={async (oldName, newName) => {
                  const all = await db.getAllHabits();
                  const targets = all.filter(h => h.category === oldName);
                  for (const h of targets) await db.updateHabit({ ...h, category: newName });
                  await loadLabData();
              }}
              onDeleteCategory={async (catName) => {
                  const all = await db.getAllHabits();
                  const targets = all.filter(h => h.category === catName);
                  for (const h of targets) await db.updateHabit({ ...h, category: 'Uncategorized' });
                  await loadLabData();
              }}
              onDeleteHabitFromDB={handleDeleteHabitFromLibrary}
              maxCycles={maxCycles}
