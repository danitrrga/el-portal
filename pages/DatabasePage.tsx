...
import { Version, Cycle, EntityStatus, Habit, Goal, HabitWeight, GoalType } from '../types';
...
const EditModal: React.FC<{ 
    ...
    categories?: string[];
}> = ({ isOpen, onClose, type, data, onSave, categories }) => {
...
const DatabasePage: React.FC = () => {
  ...
  const loadData = async () => {
      const v = JSON.parse(localStorage.getItem('elportal_versions') || '[]');
      const c = JSON.parse(localStorage.getItem('elportal_cycles') || '[]');
      const h = await db.getAllHabits();
  ...
  const handleDelete = async (id: string, type: TabType) => {
      ...
      try {
          switch (type) {
              case 'versions': await db.deleteVersion(id); break;
              case 'cycles': await db.deleteCycle(id); break;
              case 'habits': await db.deleteHabit(id); break;
              case 'goals': await db.deleteGoal(id); break;
          }
          setVerifyDeleteId(null);
          loadData();
      } catch (e) {
  ...
  const handleSaveEdit = async (updatedData: any) => {
      ...
      switch (type) {
          case 'versions': await db.updateVersion(updatedData); break;
          case 'cycles': await db.updateCycle(updatedData); break;
      ...
      if (action === 'delete') {
          ...
          if (action === 'create') {
              const newGoal: Goal = {
                  ...
                  type: payload.type,
                  subtasks: payload.subtasks?.map((s: string) => ({ name: s.trim(), done: false })) || []
              };
              ...
              if (originalGoal) {
                  ...
                  if (payload.subtasks && typeof payload.subtasks[0] === 'string') {
                       updatedGoal.subtasks = payload.subtasks.map((s: string) => ({ name: s.trim(), done: false }));
                  }
                  await db.updateGoal(updatedGoal);
              }
          }
  ...
  const handleCycleManageHabit = async (action: 'create' | 'update' | 'delete', payload: any) => {
      ...
      if (action === 'delete') {
          ...
          if (originalHabit) {
              await db.updateHabit({ ...originalHabit, ...payload });
          }
      }
      loadData();
  };
          ...
          case 'cycles':
              ...
              rows = cycles.map(c => ({
                  ...
                  data: [
                      ...
                      <div className="flex flex-col gap-1">{c.focus_priorities.slice(0,2).map((p,i) => <span key={i} className="text-xs">• {p}</span>)}</div>,
          ...
          case 'habits':
              ...
              rows = habits.map(h => ({
                  ...
                  data: [
                      ...
                      <div className="flex gap-0.5">{[...Array(h.weight)].map((_,i) => <div key={i} className="h-1.5 w-1.5 rounded-full bg-pacific-500"/>)}</div>,
