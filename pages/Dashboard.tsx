...
const Dashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const historyData = await db.getHistoryData();
      setData(dashboardData);
      setHistory(historyData.dailyScores); 
      setCurrentScore(db.calculateDailyScore(dashboardData.habits, dashboardData.todayLogs));
      setLoading(false);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleToggleHabit = async (habitId: string, currentStatus: boolean) => {
    ...
    const newScore = db.calculateDailyScore(data.habits, updatedLogs);
    ...
    // Server Sync
    await db.toggleHabit(habitId, newStatus);
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                            <div className={`h-4 w-4 flex-shrink-0 rounded flex items-center justify-center transition-all duration-300 border ${
                                isDone 
               ...
               <div className="space-y-3">
                 {activeProjects.map((project) => {
                     ...
                     return (
                         <div 
                             key={project.id} 
                             className={`group relative rounded-xl border transition-all duration-300 overflow-hidden backdrop-blur-sm ${
                                 isExpanded || isDone
                                          ...
                                          {subtasks.length > 0 ? subtasks.map((task, i) => (
                                              <div key={i} className="flex items-start gap-2 group/task" onClick={(e) => e.stopPropagation()}>
                                                  <button
                                                      ...
                                                      className={`mt-0.5 h-3.5 w-3.5 rounded-[4px] border flex items-center justify-center transition-all flex-shrink-0 ${
                                                          task.done 
                                                  ...
                                                  <span 
                                                      className={`text-xs leading-tight transition-colors cursor-pointer ${
                                                          task.done ? 'text-graphite-400 line-through' : 'text-graphite-600 dark:text-graphite-300'
                                                      }`}
                                                      onClick={() => handleToggleSubtask(project.id, i)}
                                                  >
                                                      {task.name}
