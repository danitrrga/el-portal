...
export const AccountSettingsModal: React.FC<AccountSettingsModalProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    db.login().then(setProfile);
  }, []);

  const handleReset = () => {
    if (confirm("WARNING: This will delete all your data and reset the application to its initial state. Are you sure?")) {
        localStorage.clear();
        ...
        <div className="space-y-6">
            {profile && (
                <div className="flex items-center gap-4 p-4 bg-graphite-50 dark:bg-graphite-800 rounded-xl border border-graphite-200 dark:border-graphite-700">
                    <img src={profile.avatar_url} className="w-12 h-12 rounded-full bg-graphite-200 object-cover" alt="Avatar"/>
                    <div>
                        <div className="text-sm font-bold text-graphite-900 dark:text-white">User ID: {profile.id}</div>
                        <div className="text-xs text-graphite-500 font-mono mt-0.5">Local Session</div>
                    </div>
                </div>
            )}
            ...
            <div className="space-y-3 pt-2">
                ...
                >
                    <Trash2 size={16}/> Hard Reset System
                </button>
                <p className="text-[10px] text-center text-graphite-400">
                    This action will clear all local storage data.
                </p>
            </div>
        </div>
