...
export const CycleEditorModal: React.FC<CycleEditorModalProps> = ({ 
   ...
   // Initialization
   useEffect(() => {
       db.getMantras().then(setAllMantras);
   ...
   const handleAddMantra = async () => { if (newMantra.trim()) { await db.addMantra(newMantra); setAllMantras(prev => [...prev, newMantra]); setSelectedMantras(prev => [...prev, newMantra]); setNewMantra(''); } };
